...
def rendijaProba(numRendijas,numBlancos,vectorProba):
    ...
    while num<numRendijas+1:
        ...
        for i in range(paredes,len(vectorProba)+paredes):
            Matriz[i][num]=vectorProba[pos][0]
            pos+=1
        num+=1
...
def rendijaCuanti(numRendijas,numBlancos,vectorProba):
    ...
    for i in range(paredes):
        for j in range(paredes):
            if i==0:
                ...
                Matriz[i][0]=(1/math.sqrt(numRendijas),0)
    ...
    while num<numRendijas+1:
        pos=0
        for i in range(ini,len(vectorProba)+ini):
            Matriz[i][num]=vectorProba[pos][0]
            pos+=1
        num+=1
        ini+=numBlancos+1
    ...
    for i in range(len(VF)):
        for j in range(len(VF[0])):
            VF[i][j]= ComplejosCal.producto(VF[i][j],ComplejosCal.conjugado(VF[i][j]))
